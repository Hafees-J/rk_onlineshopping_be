...
class Shop(models.Model):
    ...
    def __str__(self):
        return f"{self.get_shop_type_display()} - {self.name}"


class DeliveryCondition(models.Model):
    shop = models.ForeignKey(Shop, on_delete=models.CASCADE, related_name="delivery_conditions", null=True)
    ...
    def __str__(self):
        return f"{self.shop.name} Delivery Conditions" if self.shop else "Delivery Conditions"
